...
///
/// 起動時オンボーディング画面([`super::intro`])のロボット風マスコット
/// (白いヘルメット+黒バイザーに光る目+頭頂のオレンジの飾り+ティールの
/// グローブ+オレンジのブーツ)にデザインを揃えている。プレイヤーごとに
/// 変わるのは体のスーツ色(`suit`/`suit_light`)だけで、ヘルメット・バイザー・
/// グローブ・ブーツは全員共通(対戦相手同士を区別する手がかりはスーツ色)。
pub fn player_sprite_with_suit(suit: Color, suit_light: Color) -> Sprite {
    const OUTLINE: Color = Color::Rgb(18, 18, 22);
    const HELMET: Color = Color::Rgb(245, 246, 248);
    const VISOR: Color = Color::Rgb(15, 18, 26);
    const EYE_GLOW: Color = Color::Rgb(70, 220, 225);
    const CREST: Color = Color::Rgb(230, 120, 40);
    const GLOVE: Color = Color::Rgb(50, 150, 165);
    const BOOT: Color = Color::Rgb(215, 115, 35);
    Sprite::from_art(
        [
            ".....199991.....",
            "...1222222221...",
